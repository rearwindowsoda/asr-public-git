...
const createToken = (id) => {
	return jwt.sign({ id }, 'H&^165141ZXFEUYG(^&%5435sdasdf7865^&as!^&*#', {
		expiresIn: maxAge,
	});
};
	...
	.post('/signup', async (req, res) => {
		...
		try {
			...
			if (uniqueLoginValidation !== null) {
				res
					.status(400)
					.send({ message: 'Taki login istnieje już w bazie użytkowników.' });
				return;
			}
			if (
				typeof login === 'string' &&
				login.trim().length > 5 &&
				typeof unsafePassword === 'string' &&
				unsafePassword.length > 5
			) {
				...
				res
					...
					.json({
						message:
							'Twój login i hasło muszą mieć przynajmniej 6 znaków (nie licząc znaków białych).',
					});
			}
		} catch (error) {
			console.log('error');
			res
				.status(500)
				.json({
					message: 'Coś poszło nie tak, spróbuj ponownie. Przepraszamy.',
				});
		}
	})
	...
	.post('/login', async (req, res) => {
		try {
			...
			if (findUserInDb === null) {
				res
					.status(400)
					.send({ message: 'Taki login nie istnieje w bazie użytkowników.' });
				return;
			} else {
