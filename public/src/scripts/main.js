...
function showPoints() {
  ...
  pointsStatus.innerText = `Liczba pkt. ASR: ${localFetch}`;
  pointsMultiply.innerText= `Liczba pkt. ${localFetch * 30}`;
}
...
function subtractPoints(numberOfPoints){
  ...
  resetButton.addEventListener('click', function () {
    play();
    const userConfirmation = confirm('Czy na pewno chcesz zresetować dotychczasowe punkty?')
    if (userConfirmation){
